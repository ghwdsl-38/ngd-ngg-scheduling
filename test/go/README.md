...
go test -p=1 ./test/go/group5_prc_refresh_lifecycle -run '^TestGroup5_' -v -count=1
go test -p=1 ./test/go/group7_bond_topology_flow -run '^TestGroup7_' -v -count=1
```
...
```bash
ls -1dt test/go/group1_algorithm_worker/results/* | head -1
ls -1dt test/go/group2_prc_algorithm/results/* | head -1
ls -1dt test/go/group3_prc_ngd_ngg/results/* | head -1
ls -1dt test/go/group4_full_real_algorithm/results/* | head -1
ls -1dt test/go/group5_prc_refresh_lifecycle/results/* | head -1
ls -1dt test/go/group7_bond_topology_flow/results/* | head -1
```

以第一组为例，查看最新结果：

```bash
latest_result=$(ls -1dt test/go/group1_algorithm_worker/results/* | head -1)
...
```bash
source scripts/go-test-env.sh
dlv test ./test/go/group1_algorithm_worker -- -test.run '^TestGroup1_' -test.v -test.count=1
```

完整链路可使用：

```bash
dlv test ./test/go/group4_full_real_algorithm -- -test.run '^TestGroup4_' -test.v -test.count=1
```
...
```bash
cat test/go/<组目录>/results/<run-id>/comparison/diff.txt
```
...
```bash
source scripts/go-test-env.sh
UPDATE_GOLDEN=1 go test -p=1 ./test/go/group1_algorithm_worker -run '^TestGroup1_' -v -count=1
git diff -- test/go/group1_algorithm_worker/testdata/expected
```
