...
这是一套规模性能测试，不替换现有`test/go/group1~group4`中的1000 Node功能正确性测试。规模测试使用全Ready、无资源占用的确定性Node，避免节点故障和资源不足影响“精确选择N个Node”的性能测量。
...
```bash
cat test/go/scale_benchmark_3000/reports/20260825-formal-3000/summary.md
```

查看Group4的1000 Node原始30次耗时：

```bash
cat test/go/scale_benchmark_3000/group4_full_real_algorithm/results/20260825-formal-3000/samples/select-1000.csv
```

查看Group4最终1000 Node NGG：

```bash
sed -n '1,200p' test/go/scale_benchmark_3000/group4_full_real_algorithm/results/20260825-formal-3000/evidence/select-1000/ngg.yaml
```
