# PRC、Algorithm与LLDP发布镜像验证

本目录验证打包后的Docker容器，不直接启动源码中的PRC、Algorithm或LLDP。
验证结果按时间写入`image_validation/results/<run-id>/`，与`images/`目录
处于同一层级。

## 1. 发布目标

Docker Hub使用一个Repository、三个Tag：
...
7. PRC向Algorithm上传1000 Node静态快照；
...
该命令默认发布三个多架构Tag，并使用`docker buildx imagetools inspect`
