...
计时前已经完成envtest启动、CRD安装、1000 Node/Pod及拓扑标签预置和PRC Cache Sync。计时从PRC通过Watch观察到NGD并进入Reconcile开始，到NGG为`Active`结束。NGD `Fulfilled`继续作为正确性断言，但不计时。
...
1000个Node只是API对象，测试先创建Node，再通过Status子资源写入Allocatable和Ready状态。Mock Algorithm只代替算法边界：它记录PRC请求，并按确定规则返回一个包含3个具体Node的候选组。
...
启动envtest并安装CRD
  ...
  -> PRC观察到NGD，开始计时
  -> PRC读取NGD、Node、Pod并调用Mock Algorithm
...
主要断言包括真实Watch触发、PRC协议内容、正式NGG只有一组`spec.nodes`、Node来自算法rank 1，以及NGD/NGG状态正确。
