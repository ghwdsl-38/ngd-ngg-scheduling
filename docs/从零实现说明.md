...
### 4.1 Node Label：一个 Node 的三层网络拓扑
...
远端端口、本地网卡和采集时间写在 Annotation 中。旧 `NodeNetworkTopology` CRD 暂时保留，PRC 在 Label 不完整时可以回退读取，新的 Go Agent 不再写 NNT。
...
- `prc/pkg/controller/static_snapshot_controller.go`：独立Watch Node/拓扑、维护静态Hash并确认Algorithm缓存；
...
Manager内注册两个Controller：

- `NodeStaticSnapshotReconciler`监听Node和旧NNT/拓扑变化，独立构造并同步静态快照；
- `NodeGroupDemandReconciler`处理NGD，并在Node/Pod/拓扑变化时触发需要重新评估的任务。
...
独立`NodeStaticSnapshotReconciler`优先从Node及其拓扑Label构造静态快照；Label不完整时回退旧NNT。Node Watch只接受创建、删除、Label或Allocatable变化，忽略不影响静态内容的心跳/ResourceVersion更新：
...
→ 导出NGD、NGG、拓扑Node YAML、旧NNT对照、落点和日志
```
