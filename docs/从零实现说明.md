...
```mermaid
flowchart TB
    STATIC[Leaf到Border到Core静态JSON] -->|ConfigMap| AGENT[Go LLDP Agent DaemonSet]
    WORKER[9个Worker] -->|模拟种子或物理LLDP 0x88CC<br/>取得Node到Leaf| AGENT
    AGENT -->|写Leaf/Border/Core| LABEL[Node Labels]
    USER[用户或任务系统] -->|任务级资源与拓扑要求| NGD[NodeGroupDemand]
    USER -->|实际工作负载| JOB[VolcanoJob或Kubernetes Job]

    LABEL --> API[Kubernetes API Server]
    API -->|Watch Node、Pod、NGD| PRC[Go PRC<br/>controller-runtime]
    NGD --> API
    JOB -->|Controller创建| POD[待调度Pod]

    PRC -->|PUT静态Node快照<br/>内容Hash作为身份| ALG[Go Algorithm API Server<br/>HTTP + 缓存 + 编排]
    PRC -->|POST任务需求<br/>当次动态调度状态| ALG
    PROM[Prometheus] -->|Go后台周期采集| MCACHE[Go进程内指标缓存]
    MCACHE --> ALG
    ALG -->|完整上下文/本地JSON-RPC| PY[单一Python算法Worker]
    PY -->|分数降序稳定Top-3| ALG
    ALG -->|保持原顺序| PRC
...
- Worker → LLDP Agent：取得节点连接的接入交换机和端口；静态 JSON 补齐 Border/Core；
- LLDP Agent → Node Label：将完整三层拓扑写入 Kubernetes，进程重启时可直接恢复；
- API Server → PRC：PRC 从 Kubernetes 获得第一层所需的节点与任务状态；
- PRC → Algorithm：PRC 发送静态拓扑快照、动态状态和任务需求；
- Go Algorithm → Python Worker：Go 解析快照和指标并发送完整计算上下文，Python 只执行算法；
- Algorithm → PRC：Go 返回 Python 已排序的候选组，不修改算法分数和顺序；
