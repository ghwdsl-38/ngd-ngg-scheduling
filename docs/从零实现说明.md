...
flowchart TB
    ...
    API -->|步骤4a：Watch Node、Pod、NGD| PRC[Go PRC<br/>controller-runtime]
    JOB -->|步骤4b：Controller创建| POD[待调度Pod]

    PRC -->|步骤5a：PUT静态Node快照<br/>内容Hash作为身份| ALG[Go Algorithm API Server<br/>HTTP + 缓存 + 编排]
    PRC -->|步骤5b：POST任务需求<br/>当次动态调度状态| ALG
...
4. PRC Watch Node、Pod、NGD，Job Controller 创建待调度 Pod；
5. PRC 向 Go Algorithm 发送静态 Hash 快照、任务需求和当次动态状态；Go 同时读取自身缓存的 Prometheus 指标快照；
...
Controller Watch NGD、Node、Pod 和旧 NNT。Node/Pod/拓扑 Label 变化会主动把 NGD 重新入队；NNT Watch 只为迁移回退保留：
...
PRC 优先从 Node 及其拓扑 Label 构造静态快照；Label 不完整时回退旧 NNT：
