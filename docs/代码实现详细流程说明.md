...
flowchart TB
    ...
    PROM -->|每 30 秒查询 CPU/内存利用率| MCACHE
...
flowchart TB
    ...
    CACHE[MetricsCache<br/>current + previous]
...
[`cache/metrics.py`](../algorithm_api_server/algorithm_api_server/cache/metrics.py) 在 FastAPI lifespan 启动后台线程，默认每 30 秒执行两条 PromQL：
...
[`SetupWithManager()`](../prc/internal/controller/prc_controller.go) 注册了四类来源：
...
flowchart TB
    ...
    LIST[列出全部 Node、Pod、NNT]
    HASH[构建静态快照和动态状态 Hash]
    TASK{taskRef 对应任务存在吗}
    PUT[PUT 静态快照到 Algorithm]
    OLD{存在匹配的 NGG 吗}
...
[`buildStaticSnapshot()`](../prc/internal/controller/snapshot.go) 先用 NNT 的 `spec.nodeRef.uid` 建立拓扑索引，然后把每个有有效 NNT 的 Node 转成：
...
Node 按 UID/名称稳定排序，整体 JSON 规范化后计算 `sha256:` 内容 Hash。没有有效 NNT 的 Node 不进入第一层候选集；全部 Node 都缺少 NNT 时，NGD 进入 Degraded 并重试。

### 7.4 动态状态如何构造

[`buildSchedulerState()`](../prc/internal/controller/snapshot.go) 对当前全部 Node 生成：
...
当前 Go PRC 发送字段名仍是 `schedulerState`。Algorithm 的 [`RequestSchedulerState`](../algorithm_api_server/algorithm_api_server/cache/scheduler_state.py) 把它转换成请求级 `nodeUsageStates[].inUse`：
...
[`algorithm_client.go`](../prc/internal/controller/algorithm_client.go) 先调用：

```http
PUT /internal/v1/node-static-snapshots/{sha256}
```

只有 Algorithm 返回相同 `acceptedSnapshotId`，PRC 才继续。随后调用：

```http
POST /api/v1/node-groups/calculate
```

请求包含任务 UID、NGD UID/generation、静态/动态 Hash、PodSet、Node 约束、拓扑要求、算法顺序、Top-3 上限及动态状态。单次超时由 `algorithmTimeoutSeconds` 控制，默认 5 秒。
...
[`app.py`](../algorithm_api_server/algorithm_api_server/app.py) 在模块加载时创建单例 `MetricsCache` 和 `AlgorithmService`。FastAPI lifespan 启动/停止 Prometheus 后台采集线程。
...
静态快照缓存由 [`StaticNodeCache`](../algorithm_api_server/algorithm_api_server/cache/static_nodes.py) 保存 current 和 previous 两份。它会重新计算规范化 JSON Hash，路径 Hash 与内容不一致时拒绝请求。

### 8.2 一次计算如何建立上下文

[`AlgorithmService.allocate()`](../algorithm_api_server/algorithm_api_server/pipeline.py) 依次执行：
...
[`PipelineRunner`](../algorithm_api_server/algorithm_api_server/pipeline.py) 默认注册三个插件：
...
[`RequirementAlgorithm`](../algorithm_api_server/algorithm_api_server/algorithms/requirement.py) 调用 [`NodeViewBuilder`](../algorithm_api_server/algorithm_api_server/services/node_view_builder.py)，依次排除：
...
[`TopologyAlgorithm`](../algorithm_api_server/algorithm_api_server/algorithms/topology.py) 当前使用 `NarrowestFit`：
...
[`LoadBalanceAlgorithm`](../algorithm_api_server/algorithm_api_server/algorithms/loadbalance.py) 使用 [`loadbalance_profiles.json`](../algorithm_api_server/algorithm_api_server/config/loadbalance_profiles.json) 的 `balanced-v1`：
...
[`ResultBuilder`](../algorithm_api_server/algorithm_api_server/services/result_builder.py) 返回：
...
5. [`pipeline.py`](../algorithm_api_server/algorithm_api_server/pipeline.py)：理解 Algorithm 编排；
