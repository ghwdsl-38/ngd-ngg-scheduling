...
[`cache/metrics.py`](../algorithm_server/python/algorithm_worker/cache/metrics.py) 在 FastAPI lifespan 启动后台线程，默认每 30 秒执行两条 PromQL：
...
当前 Go PRC 发送字段名仍是 `schedulerState`。Algorithm 的 [`RequestSchedulerState`](../algorithm_server/python/algorithm_worker/cache/scheduler_state.py) 把它转换成请求级 `nodeUsageStates[].inUse`：
...
[`app.py`](../algorithm_server/python/algorithm_worker/app.py) 在模块加载时创建单例 `MetricsCache` 和 `AlgorithmService`。FastAPI lifespan 启动/停止 Prometheus 后台采集线程。
...
静态快照缓存由 [`StaticNodeCache`](../algorithm_server/python/algorithm_worker/cache/static_nodes.py) 保存 current 和 previous 两份。它会重新计算规范化 JSON Hash，路径 Hash 与内容不一致时拒绝请求。

### 8.2 一次计算如何建立上下文

[`AlgorithmService.allocate()`](../algorithm_server/python/algorithm_worker/pipeline.py) 依次执行：
...
[`ResultBuilder`](../algorithm_server/python/algorithm_worker/services/result_builder.py) 返回：
