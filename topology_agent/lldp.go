...
func receiveLLDP(ctx context.Context, selections map[string]interfaceSelection, bindSingle bool, timeout time.Duration, count int) (result []lldpNeighbor, returnErr error) {
	started := time.Now()
	...
	// Match lldp-new-3: a literal single interface is bound in the kernel; auto,
	// Bond-master expansion and multiple-interface scopes use one protocol
	// socket and filter the selected ingress ifindexes in userspace.
	if bindSingle && len(selections) == 1 {
		...
		log.Printf("[LLDP-AGENT] SKIP unix.Bind reason=auto-bond-or-multiple-interface-scope kernelScope=all userspaceFilter=selected-ifindexes")
	}
